...
let peopleName = person.map((person) => person.name);
...
console.log(peopleName.filter(name.startWith("j") || name.startWith("P")
)
);
...
peopleName.filter(
(name) =>
name.startWith("A") || name.startsWith("C")
).length
);
...
console.log(
persons.filter((person) => person.sex == "F").filter(person => person.name.startWith("C") ||
person.name.startWith("J") 
)
);
...
let femalePersons = person.filter(
person => person.sex === "F"
);
// Filter all the male from persons array and store in malePersons array
let malePersons = person.filter(
person => person.sex === "M"
);
...
(person) =>
person.name.startWith("J") ||
person.name.startWith("P")
);
let gradeWithNameJorP = nameWithJorP.map(
(person) => person.grade
);
